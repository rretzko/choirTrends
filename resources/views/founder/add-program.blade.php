<x-layouts.app :title="__('Add Program for User')">
    <div class="flex h-full w-full flex-1 flex-col gap-6">
        <flux:heading size="xl">{{ __('Add Program for User') }}</flux:heading>
        ...
        <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-zinc-900 p-6">
            <flux:heading size="lg" class="mb-4">{{ __('Upload Concert Program on Behalf of User') }}</flux:heading>
            <p class="text-sm text-zinc-600 dark:text-zinc-400 mb-6">
                {{ __('Select a user, then upload a concert program file (PDF, image, or text) or provide URLs to concert program pages.') }}
            </p>

            <form id="program-upload-form" action="{{ route('founder.addProgram.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                ...
                <div>
                    <flux:field>
                        ...
                        <div class="flex gap-3 items-start">
                            <select
                                name="target_user_id"
                                class="flex-1 rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 px-3 py-2 text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
                            >
                                <option value="">{{ __('-- Select a user --') }}</option>
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" @selected(old('target_user_id', $targetUserId) == $user->id)>
                                        {{ $user->alpha_name }} ({{ $user->email }})
                                    </option>
                                @endforeach
                            </select>
                            <flux:modal.trigger name="create-user">
                                <flux:button variant="ghost" type="button">{{ __('New User') }}</flux:button>
                            </flux:modal.trigger>
                        </div>
        ...
        <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-zinc-900 p-6">
            ...
            @elseif(isset($analysis['status']) && $analysis['status'] === 'failed')
                ...
                <div class="rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 p-6">
                    ...
                    <flux:button variant="ghost" href="{{ route('founder.addProgram') }}" class="mt-4">
                        {{ __('Try Again') }}
                    </flux:button>
                </div>
            @elseif(isset($analysis['status']) && $analysis['status'] === 'completed' && isset($analysis['data']))
                ...
                @endphp
                <form action="{{ route('founder.addProgram.confirm') }}" method="POST" class="space-y-4">
                    @csrf
                    @php
                        $data = $analysis['data'];
                    @endphp
                    ...
                    <flux:field>
                        ...
                        <flux:input
                            type="text"
                            name="director_name"
                            value="{{ old('director_name', $data['director_name'] ?? $targetUser?->name) }}"
                                        ...
                                        <div class="flex-1 space-y-3">
                                            ...
                                            <flux:field>
                                                ...
                                                <flux:input
                                                    ...
                                                    value="{{ $ensemble['director'] ?? $data['director_name'] ?? $targetUser?->name }}"
                    ...
                    <div class="flex justify-end gap-3 pt-4">
                        <flux:button variant="ghost" href="{{ route('founder.addProgram') }}">
                            {{ __('Start Over') }}
                        </flux:button>
                    ...
                    <div class="flex-1 space-y-3">
                        ...
                        <div>
                            ...
                            <input
                                type="text"
                                name="ensembles[${ensembleIndex}][director]"
                                value="{{ $data['director_name'] ?? $targetUser?->name }}"
                                placeholder="{{ __('Director for this ensemble') }}"
                                class="w-full rounded-lg border border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 px-3 py-2 text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
                            />
                        </div>
