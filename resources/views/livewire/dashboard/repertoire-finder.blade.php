    ...
    <div class="mx-auto max-w-2xl space-y-4">
        @if (! $isRecentUpload)
            <flux:callout icon="exclamation-triangle" color="amber">
                <flux:callout.heading>{{ __('Keep your unlimited access') }}</flux:callout.heading>
                <flux:callout.text>
                    @if ($lastUploadDate)
                        {{ __('Your last program was uploaded on :date. Continue to receive unlimited queries by uploading at least one program every six months.', ['date' => $lastUploadDate->format('l, F j, Y')]) }}
                    @else
                        {{ __("You haven't uploaded a program yet. Upload at least one program every six months to receive unlimited queries.") }}
                    @endif
                </flux:callout.text>
            </flux:callout>
        @endif

        @if ($remainingQueries === null || $remainingQueries > 0 || $aiSearching || $aiResult)
                    ...
                    @if ($remainingQueries !== null)
                        <span class="text-xs text-zinc-500 dark:text-zinc-400">
                            @if ($remainingQueries === 1)
                                {{ __('1 query left') }}
                            @else
                                {{ __(':count queries left', ['count' => $remainingQueries]) }}
                            @endif
                        </span>
                    @endif
                </div>
        ...
        @else
            <div class="rounded-2xl border border-teal-200 bg-teal-50 p-6 text-center dark:border-teal-800 dark:bg-teal-950/50">
                <flux:heading size="lg">{{ __("You've used your queries for now") }}</flux:heading>
                ...
                <flux:button href="{{ route('addProgram') }}" variant="primary" class="mt-4">
                    {{ __('Add Program') }}
                </flux:button>
            </div>
        @endif
